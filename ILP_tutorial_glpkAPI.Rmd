...
do_glpkAPI = function(
  ...
  # of length M which will not interact with the value of the objective function
  ...
  bound_type = c(rep(c(3,3,2), times = M),
                 3, #area has an upper bound
                 2, #conservation value has a lower bound
                 2) #compactness has a lower bound
  ...
  glpkAPI::setMIPParmGLPK(parm = MIP_GAP, val = gap)
  
  # Stop after specified number of seconds, convert to milliseconds
  if (is.finite(time_limit)) {
    glpkAPI::setMIPParmGLPK(parm = TM_LIM, val = 1000 * time_limit)
  }
  
  #############################
  ## Solve Model
  #############################
  t <- system.time({ screen_out = glpkAPI::solveMIPGLPK(lp = model) })
...
for(b in 1:length(compactness_scen)){
  ...
  while(!(status %in% 9:10)){ #10: unfeasible solution 
